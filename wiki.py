...
if settingdic["db"] == "mariadb" or settingdic["db"] == "mysql":
    ...
    conn = pymysql.connect(
        host=settingdic["hostname"],
        port=int(settingdic["port"]),
        user=settingdic["id"],
        passwd=settingdic["passwd"],
        db=settingdic["dbname"],
        charset="utf8",
        autocommit=True,
    )
else:
    if settingdic["db"] == "sqlite3" or settingdic["db"] == "sqlite":
        conn = sqlite3.connect(
            settingdic["dbname"] + ".db", check_same_thread=False, isolation_level=None
        )
    else:
...
class auth:
    ...
    def encode(self, username, usertype):
        epochnow = time.time()
        temp = str(self.auth.dumps({"token_type": "auth","username": username,"issued": epochnow,"usertype": usertype}))[2:-1]
        return temp
...
curs.execute(
    "create table if not exists user(userid text, pw text, acl text, date text, email text, login text, salt text)"
)
...
curs.execute(
    "create table if not exists ban(type text,username text,block text,end text,reason text,band text)"
)
...
def hashpass(password, salt):
    data = password + salt
    hash = hashlib.sha384(
        bytes(hashlib.sha512(bytes(data, "utf-8")).hexdigest(), "utf-8")
    ).hexdigest()
    return hash
    ...
    if "login" in session:
        if "email" in session:
            if tokencheck(session["login"]):
                ...
                imageurl = (
                    "https://www.gravatar.com/avatar/" + hashed_email + "?s=40&d=retro"
                )
                # print("user:"+str(tokencheck(session['login']))+":"+imageurl)
                return render_template(
                    skin + "/index.html", wikiname=wiki, imageurl=imageurl, form=form
                )
            else:
    ...
    if "login" in session:
        if tokencheck(session["login"]):
            curs.execute(
                ...
                (
                    codecs.encode(os.urandom(64), "hex").decode(),
                    tokencheck(session["login"]),
                ),
            )
            session["email"] = None
            session["login"] = None
...
def signup():
    ...
    if psw == pswck and not curs.fetchall():
        ...
        curs.execute(
            "insert into user(userid,pw,acl,date,email,login,salt) values (?,?,?,?,?,?,?)",
            (id, hash, "user", gettime(), email, logintoken, salt),
        )
...
def pagerender(pagename):
    ...
    if "login" in session:
        if tokencheck(session["login"]):
            curs.execute(
                "select acl from user where userid = (?)",
                [tokencheck(session["login"])],
            )
            if curs.fetchall():
                curs.execute(
        ...
        if acltest(pagename, "read", useracl):
            if "login" in session and "email" in session:
                if tokencheck(session["login"]):
                    ...
                    return render_template(
                        skin + "/index.html", wikiname=wiki, data=output, form=form
                    )
            else:
                return render_template(
                    skin + "/index.html", wikiname=wiki, data=output, form=form
...
def editpage(pagename):
    ...
    if curs.fetchall():
        ...
        data = "#None"
    form = (
        """<form method="POST" class="form-control" id="editer">
    <textarea name="editform" wrap="soft" rows="20" cols="40" style="width:90%; id="editform">"""
        + data
        + """</textarea>
    <br>"""
        + """<input class="btn btn-primary" type="submit" value="전송"></form><script>CKEDITOR.replace('editform',{customConfig: '/statics/config/ckeditor_config.js'});</script>"""
    )
    if "login" in session:
        if "email" in session:
            if tokencheck(session["login"]):
            ...
            return render_template(
                skin + "/index.html", wikiname=wiki, data=form, form=form_, editer=True
            )
    else:
        return render_template(
            skin + "/index.html", wikiname=wiki, data=form, form=form_, editer=True
        ...
        if apiacl["GETRAW"]:
            try:
                ...
                if pagedata and acltest(pagename, "read", curs.fetchall()[0][0]):
                    data = json.dumps(
                        {"success": True, "request": pagename, "data": pagedata[0][0]},
                        indent=2,
                    )
                else:
                    ...
                    data = json.dumps(
                        {
                            "success": True,
                            "request": pagename,
                            "data": parser_kiwi(pagename, pagedata[0][0]),
                        },
                        indent=2,
                    )
                else:
...
def postapi(apitype):
    if (
        not request.json
        or not "apikey" in request.json
        or not "pagename" in request.json
    ...
    if apitype == "EDITRAW":
        ...
        if not apiacl["EDITRAW"]:
            ...
            if temp:
                ...
                useracl = curs.fetchall()[0][0]
                if acltest(pagename, "edit", useracl) and acltest(
                    pagename, "read", useracl
                ):
...
def aclview(pagename):
    ...
    if "email" in session and "login" in session:
        ...
        return render_template(
            skin + "/acl.html", acldata=acljob, imageurl=imageurl, form=form, skin=skin
        )
...
def authorize_token():
    ...
    if hashpass(str(pwd), salt) == password:
        ...
        return (
            jsonify(
                {"success": True, "token": authfunc.encode(username, usertype)}
            ),
            200,
        )
    else:
